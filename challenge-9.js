...
class Cart {
  ...
  addItem(obj) {
    let index = this.items.findIndex(item => item.id === obj.id);
    if( index !== -1 ) {
      this.items[index].quantity++;
    } else {
...
myCart.updateQuantity(3,8);
console.log("after updating quantity: ", myCart);

myCart.addItem({
  id: 5,
  product: 'Paint Winner',
  price: 100.00,
  quantity: 2,});
...
console.log("after adding items: ", myCart)
